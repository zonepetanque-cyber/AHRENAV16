        ...
        <div className="px-5 pt-4 pb-2">
          <div className="flex items-center justify-between mb-1">
            ...
            <div className="flex bg-zinc-900 rounded-xl p-1 gap-1">
              ...
              >Tout</button>
              <button onClick={() => setFilter('cette-semaine')}
        ...
        {liveNow.length > 0 && (
          <div className="px-5 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {liveNow.map(v => (
                <button key={v.id} onClick={() => onVideoSelect(v)}
                  className="flex items-center gap-4 bg-red-600/10 border border-red-600/30 rounded-2xl p-3 hover:bg-red-600/20 transition-all group text-left w-full">
                  ...
                  <ChevronRight size={18} className="text-white/30 group-hover:text-white/70 flex-shrink-0"/>
                </button>
              ))}
            </div>
          </div>
            ...
            {sortedDays.map(day => {
              ...
              return (
                <div key={day}>
                  ...
                  {/* Émissions — grille 1 col mobile, 2 col desktop */}
                  <div className="px-5 grid grid-cols-1 md:grid-cols-2 gap-2">
                    {dayVideos.map(v => {
                      ...
                      return (
                        <button key={v.id} onClick={() => onVideoSelect(v)}
                          className={`flex items-center gap-3 rounded-2xl p-3 transition-all group text-left w-full ${isPast ? 'opacity-40' : 'hover:bg-white/5'} border border-white/5 hover:border-white/15`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <span className={`text-[9px] font-black px-2 py-0.5 rounded-full text-white inline-block mb-1 ${getChannelColor(v.channelName)}`}>
                              {v.channelName}
                            </span>
                            <p className="text-white font-bold text-[12px] leading-snug line-clamp-2">{v.title}</p>
                          </div>
                          <div className="flex-shrink-0">
                            {isPast ? <Play size={16} className="text-white/20"/> : <Bell size={16} className="text-white/20 group-hover:text-amber-400 transition-colors"/>}
                          </div>
                        </button>
                      );
                    })}
